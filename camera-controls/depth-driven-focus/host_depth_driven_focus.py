...
class DepthDrivenFocus(dai.node.HostNode):
    ...
    def build(self, preview: dai.Node.Output, control_queue: dai.Node.Output, face_detection: dai.Node.Output, face_detection_depth: dai.Node.Output) -> "DepthDrivenFocus":
        self.link_args(preview, face_detection, face_detection_depth)
        self.control_queue = control_queue
        self.sendProcessingToPipeline(True)
        return self

    def process(self, message: dai.ImgFrame, face_detection: dai.SpatialImgDetections, detection_depth: dai.ImgFrame) -> None:
        ...
        if closest_dist != 99999999:
            self._text.putText(frame, "Face distance: {:.2f} m".format(closest_dist / 1000), (330, 1045))
            new_lens_pos = max(self._lensMin, min(get_lens_position(closest_dist), self._lensMax))
        ...
        if key == ord('q'):
            ...
            if key == ord(','): self._lensPos -= LENS_STEP
            if key == ord('.'): self._lensPos += LENS_STEP
