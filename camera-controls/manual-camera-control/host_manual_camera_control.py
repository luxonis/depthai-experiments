...
class ManualCameraControl(dai.node.HostNode):
    ...
    def build(
        self,
        preview_isp: dai.Node.Output,
        preview_raw: dai.Node.Output,
    ...
    def process(self, preview_isp: dai.ImgFrame, preview_raw: dai.ImgFrame) -> None:
        ...
        if key == ord("c"):
            ...
            print(
                "Setting manual exposure, time:", self._exp_time, "iso:", self._sens_iso
            )
            ...
            if self._other_control_idx is None:
                ...
                OtherControls(self._other_control_idx) == OtherControls.AE_COMPENSATION
            ):
                ...
                OtherControls(self._other_control_idx)
                == OtherControls.ANTI_BANDING_MODE
            ):
                count = len(AntiBandingMode)
                self._anti_banding_mode_idx = (
                    self._anti_banding_mode_idx + change
                ) % count
                anti_banding_mode = list(AntiBandingMode)[
                    self._anti_banding_mode_idx
                ].value
    ...
    def update_output(self, frame: dai.ImgFrame, raw: bool) -> np.ndarray:
        ...
        if raw:
            ...
            if (
                data.size == 5312 * (3692 + 18) * 5 // 4
                or data.size == 3840 * (2160 + 22) * 5 // 4
            ):
