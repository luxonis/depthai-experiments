...
class BoxEstimatorO3d:
    ...
    def process_pcl(self, raw_pcl: dai.PointCloudData):
        ...
        if not self.is_calibrated:
            ...
            plane_inliers = self.get_plane_inliers(
                plane_eq, np.asarray(self.roi_pcl.points), DISTANCE_THRESHOLD_PLANE
            )

        # Check if there is a reasonable box even
        box = self.get_box_pcl(plane_eq, plane_inliers)
        if box is None:
            return 0, 0, 0  # No box
