...
class FilterWindow:
    def __init__(self, _lambda, _sigma) -> None:
        ...
        cv2.createTrackbar(
            "Lambda", "wlsFilter", 0, 255, self.on_trackbar_change_lambda
        )
...
class WLSFilter(dai.node.HostNode):
    def __init__(self) -> None:
        ...
        self.right_frame = self.createOutput(
            possibleDatatypes=[
                dai.Node.DatatypeHierarchy(dai.DatatypeEnum.ImgFrame, True)
            ]
        )
        self.disparity_frame = self.createOutput(
            possibleDatatypes=[
                dai.Node.DatatypeHierarchy(dai.DatatypeEnum.ImgFrame, True)
            ]
        )
    ...
    def process(self, disparity_frame: dai.ImgFrame, right_frame: dai.ImgFrame) -> None:
        ...
        self.right_frame.send(
            self.create_img_frame(right_frame, dai.ImgFrame.Type.GRAY8)
        )
        self.disparity_frame.send(
            self.create_img_frame(disparity_frame, dai.ImgFrame.Type.RAW8)
        )
        self.depth_frame.send(
            self.create_img_frame(depthFrame, dai.ImgFrame.Type.RAW10)
        )
        self.filtered_disp.send(
            self.create_img_frame(filteredDisp, dai.ImgFrame.Type.RAW8)
        )
        self.colored_disp.send(
            self.create_img_frame(coloredDisp, dai.ImgFrame.Type.BGR888p)
        )
