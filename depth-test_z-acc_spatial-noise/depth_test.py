	...
	def save_results(self, name):
		...
		if not config.resuls_file.exists():
			with open(config.resuls_file, "w") as f:
				f.write("name,ground_truth,z_accuracy,spatial_noise,subpixel_spatial_noise,fillrate,vertical_tilt,horizontal_tilt\n")
		with open(config.resuls_file, "a") as f:
			f.write(f"{name},{self.camera_wall_distance},{np.mean(self.z_accuracy_medians) / self.camera_wall_distance * 100:.2f},{np.mean(self.spatial_noise_rmses)*1000:.2f},{np.mean(self.subpixel_spatial_noise_rmses):.2f},{np.mean(self.fillrate_values)*100:.2f},{vertical_tilt * 180/np.pi},{horizontal_tilt  * 180/np.pi}\n")
