...
class BoxEstimator:
    def __init__(self, median_window=3, threshold=50):
        """
        Box estimator helper class for multiple boxes.
        ...
        # Median filter for box dimensions
        self.median_window = median_window
        self.prev_num_detections = 0
        self.prev_dimensions_dict = {}

        self.pcd = o3d.geometry.PointCloud()

        self.corners = None

    def process_points(self, plane_eq, points_roi: np.ndarray, index: int, num_detections: int) -> Tuple:
        ...
        self.pcd = self.pcd.remove_statistical_outlier(20, 0.1)[0] 
        ...
        # For median filtering of the dimensions
        if num_detections != self.prev_num_detections:
            self.prev_dimensions_dict.clear()  # Clear the dimensions dictionary
            self.prev_num_detections = num_detections  # Update previous number of detections

        if index not in self.prev_dimensions_dict:
            self.prev_dimensions_dict[index] = []  
    
        self.prev_dimensions_dict[index].append(dimensions)

        corners = self.get_3d_corners()
        return self._filtered_dimensions(index), corners
    ...
    def _filtered_dimensions(self, index):
        # If too many dimensions, remove the oldest one
        if len(self.prev_dimensions_dict[index]) > self.median_window:
            self.prev_dimensions_dict[index].pop(0)

        # Get median of the dimensions
        length = np.median([d[0] for d in self.prev_dimensions_dict[index]])
        width = np.median([d[1] for d in self.prev_dimensions_dict[index]])
        height = np.median([d[2] for d in self.prev_dimensions_dict[index]])
