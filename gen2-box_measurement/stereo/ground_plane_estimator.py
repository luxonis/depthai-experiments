...
class GroundPlaneEstimator:
    def __init__(self, distance_threshold=30, n_points_sampled_plane=3, max_iterations=300, voxel_size=25):
    ...
    def check_if_ground(self, gravity_vect, plane_eq):
        ...
        # Camera and IMU c.s are not aligned; Align gravity vector with camera c.s. Values for OAK-D-PRO  
        R = np.array([[0, 1, 0],
                    [1, 0, 0],
                    [0, 0, -1]])
        aligned_gravity_vect = np.dot(R, gravity_vect)
