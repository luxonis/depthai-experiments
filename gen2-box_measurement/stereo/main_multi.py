    ...
    while oak.running():
        ...
        if frame_counter % frame_skip == 0:
            ...
            for index, det in enumerate(nn.detections):
                ...
                # to show on Cropped vindow
                lines = box.get_3d_lines(corners)
                all_lines_pc.append(lines)
                corners = box.inverse_corner_points()  
                # to show on Right window 
                lines = box.get_3d_lines(corners)
            ...
            if len(all_box_pcl) >= 1:
                ...
                viewer.log_line_segments(f"Cropped/Box_Edges", np.vstack(all_lines_pc), stroke_width=4, color=(1.0,0,0.0))
