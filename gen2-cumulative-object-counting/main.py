...
def callback(packet):
    ...
    visualizer.add_line(pt1=pt1, pt2=pt2, color=(255, 255, 255), thickness=1)

    visualizer.add_text(counter_str, outline=True, position=TextPosition.MID_LEFT, size=1.2)
...
with OakCamera() as oak:
    ...
    nn = oak.create_nn('yolov7tiny_coco_640x352', color, tracker=True)

    nn.config_tracker(
        tracker_type=TrackerType.ZERO_TERM_COLOR_HISTOGRAM,
        track_labels=[0],
        assignment_policy=TrackerIdAssignmentPolicy.UNIQUE_ID
    )
