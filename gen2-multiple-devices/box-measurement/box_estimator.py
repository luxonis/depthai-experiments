...
N_POINTS_SAMPLED_PLANE = (
    3  # Defines the number of points that are randomly sampled to estimate a plane
)
...
class BoxEstimator:
    ...
    def detect_ground_plane(self):
        points = o3d.utility.Vector3dVector(
            np.array(
                [
                    [
                        config.point_cloud_range["x_min"],
                        config.point_cloud_range["y_min"],
                        config.point_cloud_range["z_min"],
                    ],
                    [
                        config.point_cloud_range["x_max"],
                        config.point_cloud_range["y_max"],
            ...
            np.array(
                [
                    ...
                    [
                        config.point_cloud_range["x_max"],
                        config.point_cloud_range["y_max"],
                        config.point_cloud_range["z_max"],
                    ],
                ]
            )
        )
    ...
    def get_box_top(self):
        ...
        top_plane_eq, top_plane_inliers = self.fit_plane_vec_constraint(
            [0, 0, 1], points_np, 0.01, 30
        )
            ...
            np.array(
                [
                    ...
                    [
                        config.point_cloud_range["x_max"],
                        config.point_cloud_range["y_max"],
                        self.height - config.min_box_height,
                    ],
                ]
            )
        )
    ...
    def get_dimensions(self):
        ...
        # np.save("oak-examples/sample_data/coordinates.npy", coordinates)
