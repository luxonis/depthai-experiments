...
class NodeGraph(QtCore.QObject):
    ...
    def _serialize(self, nodes):
        ...
        for n in nodes:
            ...
            # PIPELINE GRAPH: Remove extraneous data
            del node_dict[n.model.id][ "color" ]
            del node_dict[n.model.id][ "border_color" ]
            del node_dict[n.model.id][ "text_color" ]
            del node_dict[n.model.id][ "selected" ]
    ...
    def _deserialize(self, data, relative_pos=False, pos=None, set_parent=True):
        ...
        for n_id, n_data in data.get('nodes', {}).items():
            ...
            if NodeCls:
                ...
                for prop, val in n_data.get('custom', {}).items():
                    ...
                    if node.view.has_widget( prop ):
                        node.view.get_widget( prop ).set_value( val )
        ...
        for connection in data.get('connections', []):
            ...
            in_port = in_node.inputs().get(pname)
            if in_port is None:
                in_port = in_node.add_input( pname )
            ...
            out_port = out_node.outputs().get(pname) 
            if out_port is None:
                out_port = out_node.add_output( pname )
