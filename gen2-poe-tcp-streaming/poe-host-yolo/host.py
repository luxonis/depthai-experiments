...
try:
    COLOR = (127,255,0)
    img_interleaved = None
    while True:
        header = str(sock.recv(64), encoding="ascii")
        ...
        elif chunks[0] == "DETECTION" and img_interleaved is not None:
            print(f">{header}<")
            class_id = int(chunks[1])
            confidence = float(chunks[2])
            bbox = [
                int(float(chunks[3]) * img_interleaved.shape[1]),
                int(float(chunks[4]) * img_interleaved.shape[0]),
                int(float(chunks[5]) * img_interleaved.shape[1]),
                int(float(chunks[6]) * img_interleaved.shape[0])
            ]
