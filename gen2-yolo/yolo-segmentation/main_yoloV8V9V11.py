  ...
  with dai.Device() as device:
    ...
    while True:
      ...
      if nn_out is not None:
        ...
        dims0 = layers[0].dims            # (1, 116, 8400)
        output0 = output0.reshape(dims0)
        output0 = output0.squeeze(0)      # (116, 8400)
        ...
        filtered = class_scores > 0.0
        filtered_boxes = bounding_boxes[:, filtered]
        filtered_scores = class_scores[filtered]
        filtered_class_ids = class_ids[filtered]
        filtered_mask_weights = mask_weights[:, filtered]
        ...
        indices = cv2.dnn.NMSBoxes(bboxes.tolist(), filtered_scores.tolist(), confidence_threshold, iou_threshold)

        final_boxes = [[int(v) for v in bboxes[i]] for i in indices]
        final_scores = [filtered_scores[i] for i in indices]
