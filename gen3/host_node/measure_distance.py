...
class MeasureDistance(dai.node.HostNode):
    ...
    def build(self, depth_frames: dai.Node.Output, calib_data : dai.CalibrationHandler, roi: RegionOfInterest) -> "MeasureDistance":
    ...
    def process(self, depth_frame: dai.ImgFrame) -> None:
        ...
        HFOV: float = np.deg2rad(self._calib_data.getFov(dai.CameraBoardSocket(depth_frame.getInstanceNum()), useSpec=False))
        ...
        centroid = Point2d(int((self._roi.xmax + self._roi.xmin) / 2), int((self._roi.ymax + self._roi.ymin) / 2)) # Get centroid of the ROI
        ...
        spatial_distance = SpatialDistance(centroid, dai.Point3f(averageDepth * math.tan(angle_x), -averageDepth * math.tan(angle_y), averageDepth))
