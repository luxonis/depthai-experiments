...
class BboxProcessing(dai.node.HostNode):
    def __init__(self) -> None:
        super().__init__()
        self.output_img = dai.Node.Output(self, possibleDatatypes=[dai.Node.DatatypeHierarchy(dai.DatatypeEnum.ImgFrame, True)])
        self.output_bboxes = dai.Node.Output(self)

    
    def build(self, img_frames: dai.Node.Output, nn_data: dai.Node.Output) -> "BboxProcessing":
