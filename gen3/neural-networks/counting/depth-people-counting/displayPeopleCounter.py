...
class DisplayPeopleCounter(dai.node.HostNode):
    ...
    def build(self, depth_in : dai.Node.Output, tracklets_in : dai.Node.Output, 
              det_in_q : dai.InputQueue, frame_in_q : dai.InputQueue,
              disparity_multiplier : float) -> "DisplayPeopleCounter":
    ...
    def get_detections(self, contours, depth_rgb):
        ...
        if len(contours) != 0:
            ...
            if 15000 < area:
                ...
                det.label = 1
                det.confidence=1.0
                det.xmin, det.ymin, det.xmax, det.ymax = self.frame_norm(depth_rgb, (x, y, x+w, y+h))
                dets.detections = [det]
