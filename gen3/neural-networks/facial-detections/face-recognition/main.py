...
with dai.Pipeline() as pipeline:
    ...
    color_q = cam.video.createOutputQueue()
    color_q.setName("color")
    ...
    detection_q = face_det_nn.out.createOutputQueue()
    detection_q.setName("detection")
    ...
    recognition_q = face_rec_nn.out.createOutputQueue()
    recognition_q.setName("recognition")
    ...
    # Output queues
    queues = [color_q, detection_q, recognition_q]
    ...
    while pipeline.isRunning():
        for queue in queues:
            ...
            if queue.has():
                sync.add_msg(queue.get(), queue.getName())
        ...
        if msgs is not None:
            ...
            for i, detection in enumerate(dets):
                ...
                firstLayer = msgs["recognition"][i].getAllLayerNames()[0]
                features = msgs["recognition"][i].getTensor(firstLayer).astype(np.float16).flatten()
