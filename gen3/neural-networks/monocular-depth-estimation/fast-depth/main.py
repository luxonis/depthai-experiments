...
from host_fast_depth import FastDepth

parser = argparse.ArgumentParser()
parser.add_argument('-nn', '--nn-choice', type=str
                    , help="Choose between 2 neural network models from {256x320,480x640} (the smaller one is default)")
args = parser.parse_args()

if args.nn_choice is None or args.nn_choice == "256x320":
...
with dai.Pipeline() as pipeline:
    ...
    depth = pipeline.create(FastDepth).build(
        preview=cam.preview,
        nn=nn.out,
        nn_shape=nn_shape
    )
