...
class PalmDetection(dai.node.HostNode):
    ...
    def process(self, preview: dai.ImgFrame, nn_in: dai.NNData) -> None:
        frame = preview.getCvFrame()
        if frame is not None and nn_in is not None:
            palm_coords = self.decode(frame, nn_in)
    ...
    def decode(self, frame: dai.ImgFrame, nn_data: dai.NNData):
        """
