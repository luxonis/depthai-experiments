...
modelDescription = dai.NNModelDescription(
    modelSlug="person-detection-retail",
    platform=device.getPlatform().name,
    modelVersionSlug="0013-544x320",
)
...
with dai.Pipeline(device) as pipeline:
    ...
    stereo = pipeline.create(dai.node.StereoDepth).build(
        left=left.requestOutput((640, 400)), right=right.requestOutput((640, 400))
    )
    ...
    social_distancing = pipeline.create(SocialDistancing).build(preview=rgb, nn=nn.out)
