...
modelDescription = dai.NNModelDescription(
    modelSlug="paddle-text-detection",
    platform=device.getPlatform().name,
    modelVersionSlug=f"{NN_HEIGHT}x{NN_WIDTH}",
)
...
parser.add_argument(
    "-t", "--thresh", help="set the bitmap threshold", default=0.01, type=float
)
parser.add_argument(
    "-ms", "--min_size", default=1, type=int, help="set min size of box"
)
parser.add_argument(
    "-mc",
    "--max_candidates",
...
with dai.Pipeline(device) as pipeline:
    ...
    output = cam.requestOutput(
        (NN_WIDTH, NN_HEIGHT),
        dai.ImgFrame.Type.BGR888p if rvc2 else dai.ImgFrame.Type.BGR888i,
    )
    ...
    parser = pipeline.create(TextDetectionParser).build(
        nn=detection_nn.out, nn_size=(NN_HEIGHT, NN_WIDTH)
    )
    ...
    normalize_bbox = pipeline.create(NormalizeBbox).build(
        nn=mask_to_bbox.output, frame=output, manip_mode=dai.ImgResizeMode.CROP
    )
    blur_bboxes = pipeline.create(BlurBboxes).build(
        nn=normalize_bbox.output, frame=output
    )
