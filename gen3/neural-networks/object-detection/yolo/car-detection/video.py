...
class Video:
    ...
    def _getAvailableRecordings(self, ) -> dict[str, tuple[list[str], int]]:
        """
    ...
    def _download_video(self, available_recordings: dict) -> None:
            if self.video in available_recordings:
                ...
                print("Downloading depthai recording '{}' from Luxonis' servers, in total {:.2f} MB"
                            .format(self.video, arr[1] / 1e6))
                self._downloadRecording(arr[0])
            else:
                raise ValueError(f"DepthAI recording '{self.video}' was not found on the server!")
