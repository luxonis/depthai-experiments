...
class DeviceDecoding(dai.node.HostNode):
    ...
    def process(self, img_frame: dai.ImgFrame, img_detections: dai.ImgDetections) -> None:
        frame: np.ndarray = img_frame.getCvFrame()
        cv2.putText(frame, "NN fps: {:.2f}".format(self._counter / (time.monotonic() - self._startTime)),
                    (2, frame.shape[0] - 4), cv2.FONT_HERSHEY_TRIPLEX, 0.4, self._color)
    ...
    def _displayFrame(self, name: str, frame: np.ndarray, detections: list[dai.ImgDetection]):
        color = (255, 0, 0)
        for detection in detections:
            bbox = self._frameNorm(frame, (detection.xmin, detection.ymin, detection.xmax, detection.ymax))
            cv2.putText(frame, self._labels[detection.label], (bbox[0] + 10, bbox[1] + 20), cv2.FONT_HERSHEY_TRIPLEX, 0.5, 255)
