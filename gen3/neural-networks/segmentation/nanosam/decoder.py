...
class ONNXDecoder:
    def __init__(self):
        self.session = ort.InferenceSession(DECODER_PATH, providers=["CPUExecutionProvider"])
    ...
    def _run_mask_decoder(self, features, points=None, point_labels=None, mask_input=None):
    ...
    def _upscale_mask(self, mask, image_shape):
    ...
    def predict(self, features, points, point_labels, mask_input=None, image_size=(1024, 1024)):
        points = self._preprocess_points(points, (image_size[1], image_size[0]))
        mask_iou, low_res_mask = self._run_mask_decoder(features, points, point_labels, mask_input)
