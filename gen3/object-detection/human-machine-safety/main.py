...
def crop_to_rect(frame: np.ndarray) -> np.ndarray:
...
with dai.Pipeline(device) as pipeline:
    ...
    while pipeline.isRunning():
        in_rgb: dai.ImgFrame = vid_q.tryGet()
        if in_rgb is not None:
            frame = crop_to_rect(in_rgb.getCvFrame())

        # Check for mobilenet detections
        in_det: dai.ImgDetections = det_q.tryGet()
        if in_det is not None:
            detections = in_det.detections

        in_depth: dai.ImgFrame = depth_q.tryGet()
