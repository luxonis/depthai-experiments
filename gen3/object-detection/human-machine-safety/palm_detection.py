...
class PalmDetection:
    def run_palm(self, frame, nn_data):
        ...
        anchors = np.load(str(Path(__file__).parent / Path("anchors_palm.npy")))

        # Run the neural network
        raw_box_tensor: np.ndarray = nn_data.getTensor("regressors").reshape(-1, 896, 18)  # regress
        raw_score_tensor: np.ndarray = nn_data.getTensor("classificators").reshape(-1, 896, 1)  # classification
    ...
    def raw_to_detections(self, raw_box_tensor: np.ndarray, raw_score_tensor: np.ndarray, anchors_, shape, num_keypoints):
        """
