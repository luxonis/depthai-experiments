...
with dai.Pipeline(device) as pipeline:
    ...
    cam_nn = cam_rgb.requestOutput(size=(512, 288), type=dai.ImgFrame.Type.BGR888i if platform == dai.Platform.RVC4 else dai.ImgFrame.Type.BGR888p) # 16:9 aspect ratio
    same_aspect_ratio = cam_rgb.requestOutput(size=(640, 360), type=dai.ImgFrame.Type.NV12) # 16:9 aspect ratio
    cam_full_fov = cam_rgb.requestOutput(size=(640, 480), type=dai.ImgFrame.Type.NV12) # 4:3 aspect ratio
    ...
    full_dets = pipeline.create(DrawDetections).build(frame=cam_full_fov, nn=norm_full.output, label_map=label_map)
    full = pipeline.create(Display).build(frames=full_dets.output)
    full.setName("Full cam FOV (4:3)")
    
    norm_square = pipeline.create(NormalizeBbox).build(frame=same_aspect_ratio, nn=nn.out)
    square_dets = pipeline.create(DrawDetections).build(frame=same_aspect_ratio, nn=norm_square.output, label_map=label_map)
