...
class QRScanner(dai.node.HostNode):
    ...
    def build(self, preview: dai.Node.Output, nn: dai.Node.Output, tile_positions) -> "QRScanner":
    ...
    def process(self, preview, detections) -> None:
        ...
        for det in detections.detections:
            ...
            cv2.rectangle(frame, (bbox[0], bbox[1]), (bbox[2], bbox[3]), color=box_color, thickness=2)
            ...
            # background of text
            cv2.rectangle(frame, (bbox[0], bbox[1] - text_height - baseline), 
                          (bbox[0] + text_width + 10, bbox[1]), box_color, thickness=cv2.FILLED)
            cv2.putText(frame, confidence_text, (bbox[0] + 5, bbox[1] - 5), font, font_scale, text_color, text_thickness)

            if DECODE:
                decoded_text = self.decode(frame, bbox)
                if decoded_text:
                    (decoded_text_width, decoded_text_height), decoded_baseline = cv2.getTextSize(decoded_text, font, font_scale, text_thickness)
                    
                    # background of text
                    cv2.rectangle(frame, (bbox[0], bbox[1] - decoded_text_height - baseline - decoded_text_height - decoded_baseline), 
                                  (bbox[0] + decoded_text_width + 10, bbox[1] - text_height - baseline), box_color, thickness=cv2.FILLED)
                    cv2.putText(frame, decoded_text, (bbox[0] + 5, bbox[1] - text_height - baseline - 5), font, font_scale, text_color, text_thickness)
