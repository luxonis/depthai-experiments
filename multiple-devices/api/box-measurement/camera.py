...
class Camera:
    ...
    def rgbd_to_point_cloud(self, depth_frame, image_frame):
        ...
        if config.crop_point_cloud:
            ...
            point_cloud = point_cloud.crop(
                bounding_box= o3d.geometry.AxisAlignedBoundingBox.create_from_points(points)
            )

        if config.downsample:
            point_cloud = point_cloud.voxel_down_sample(voxel_size=config.downsample_voxel_size)
