    ...
    def run(self) -> None:
        while self.isRunning():
            ...
            for i, detection in enumerate(detections_message.detections):
                ...
                # check if rec_ts is within 10ms of det_ts
                if abs(rec_ts - det_ts) < timedelta(milliseconds=200):
                    keypoints.append(keypoints_output)
                else:
                    # print difference between rec_ts and det_ts in milliseconds
                    print("Keypoints message is not in sync with detection message.")                    
                    print(f"Difference: {(rec_ts - det_ts).total_seconds() * 1000} ms")
                    print(f"Detection timestamp: {det_ts}, Keypoints timestamp: {rec_ts}")
                    break
