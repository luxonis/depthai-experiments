...
class StereoMatching(dai.node.HostNode):
    ...
    def build(self, disparity: dai.Node.Output, nn: dai.Node.Output
              ,nn_shape: tuple[int, int], max_disparity: int) -> "StereoMatching":
    ...
    def process(self, disparity: dai.ImgFrame, nn: dai.NNData) -> None:
        ...
        nn_output = nn_output.reshape((1, 2, self._nn_shape[0], self._nn_shape[1])).squeeze()
        nn_disp = nn_output[0, :, :].astype("uint8")

        nn_disp = (nn_disp * NN_DISP_MULTIPLIER * self._scale_multiplier).astype(np.uint8)
        nn_disp = cv2.resize(nn_disp, (TARGET_SHAPE[1], TARGET_SHAPE[0]))
        nn_disp_vis = cv2.applyColorMap(nn_disp, cv2.COLORMAP_INFERNO)
        nn_disp_vis = cv2.putText(nn_disp_vis, f"NN", (20, 20), cv2.FONT_HERSHEY_SIMPLEX
        ...
        stereo_disp_vis = cv2.putText(stereo_disp_vis, f"Stereo Disp", (20, 20), cv2.FONT_HERSHEY_SIMPLEX
                                      , 0.5, (255, 0, 255), 1, cv2.LINE_AA)
        print(f"nn_disp_vis.shape: {nn_disp_vis.shape}, stereo_disp_vis.shape: {stereo_disp_vis.shape}")
