...
device = dai.Device()
detection_model_description = dai.NNModelDescription(modelSlug="yolov6-nano", platform=device.getPlatform().name, modelVersionSlug="r2-coco-512x288")
detection_archive_path = dai.getModelFromZoo(detection_model_description)

recognition_model_description = dai.NNModelDescription(modelSlug="mobilenetv2-imagenet-embedder", platform="RVC2", modelVersionSlug="224x224")
...
with dai.Pipeline(device) as pipeline:
    ...
    detection_nn = pipeline.create(dai.node.DetectionNetwork).build(cam.preview, dai.NNArchive(detection_archive_path))
    ...
    deepsort_tracking = pipeline.create(DeepsortTracking).build(cam.video, detection_recognitions_sync.output, LABELS)
