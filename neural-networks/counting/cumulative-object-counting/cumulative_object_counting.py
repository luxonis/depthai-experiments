...
class CumulativeObjectCounting(dai.node.HostNode):
    ...
    def build(
        self, img_frames: dai.Node.Output, tracklets: dai.Node.Output
    ) -> "CumulativeObjectCounting":
    ...
    def _update_counter(
        ...
        if self._axis and not to.counted:
            ...
            if (
                centroid[0] > self._roi_position * width
                and direction > 0
                and np.mean(x) < self._roi_position * width
            ):
                self._counter[1] += 1
                to.counted = True
            elif (
            ...
            if (
                centroid[1] > self._roi_position * height
                and direction > 0
                and np.mean(y) < self._roi_position * height
            ):
                self._counter[3] += 1
                to.counted = True
            elif (
    ...
    def _draw_roi_line(self, frame: np.ndarray, height: int, width: int) -> None:
        if self._axis:
            cv2.line(
                frame,
                (int(self._roi_position * width), 0),
                (int(self._roi_position * width), height),
                (0xFF, 0, 0),
                5,
            )
        else:
            cv2.line(
                frame,
                (0, int(self._roi_position * height)),
                (width, int(self._roi_position * height)),
                (0xFF, 0, 0),
                5,
            )
    ...
    def _draw_count_and_status(self, frame: np.ndarray) -> None:
        ...
        if self._axis:
            ...
            cv2.putText(
                frame,
                f"Up: {self._counter[2]}; Down: {self._counter[3]}",
                (10, 35),
                font,
                0.8,
                (0, 0xFF, 0xFF),
