...
class AgeGender(dai.node.HostNode):
    ...
    def build(
        self, preview: dai.Node.Output, detections_recognitions: dai.Node.Output
    ) -> "AgeGender":
    ...
    def process(
        self, preview: dai.ImgFrame, detections_recognitions: dai.Buffer
    ) -> None:
        ...
        if detections and recognitions:
            for detection, recognition in zip(detections, recognitions):
                ...
                age = int(
                    (np.squeeze(np.array(recognition.getTensor("age_conv3").flatten())))
                    * 100
                )
