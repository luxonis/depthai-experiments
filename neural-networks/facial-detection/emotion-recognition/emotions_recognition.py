...
class DisplayEmotions(dai.node.HostNode):
    ...
    def build(
        self, rgb: dai.Node.Output, detected_recognitions: dai.Node.Output, stereo: bool
    ) -> "DisplayEmotions":
        self.stereo = stereo
    ...
    def process(
        self, rgb_frame: dai.ImgFrame, detected_recognitions: dai.Buffer
    ) -> None:
        ...
        for i, detection in enumerate(dets):
            ...
            rec = recognitions[i]
            emotion_results = self.softmax(
                rec.getFirstTensor().astype(np.float16).flatten()
            )
