...
class BatchingNeuralNetwork(dai.node.ThreadedHostNode):
    def __init__(self) -> None:
        ...
        self.out = self.createOutput(
            possibleDatatypes=[
                dai.Node.DatatypeHierarchy(dai.DatatypeEnum.NNData, True)
            ]
        )
        self._input_shape = 4

    def build(
        self, max_size: int = 4, blocking: bool = False
    ) -> "BatchingNeuralNetwork":
        self._nn_in_q = self._nn.input.createInputQueue()
        self._nn_out_q = self._nn.out.createOutputQueue(
            maxSize=max_size, blocking=blocking
        )
        return self
    ...
    def run(self):
        while self.isRunning():
            ...
            if len(data_shape) != self._input_shape:
                print(data_shape)
                raise ValueError(
                    f"Expected {self._input_shape}D tensor, got {len(data_shape)}D tensor on the first layer. Set the correct input shape with set_input_shape() method. Neural network ID: {self._nn.id}"
                )
            ...
            for batch_index in range(batch_size):
                nn_data = self._get_nn_data(
                    batched_nn_data, input_layer_names, batch_index
                )
    ...
    def _merge_nn_data(self, result_nn_data: List[dai.NNData]) -> dai.NNData:
        ...
        for output_layer in output_layer_names:
            output_tensor = np.concatenate(
                [nn_data.getTensor(output_layer) for nn_data in result_nn_data]
            )
            output_nn_data.addTensor(output_layer, output_tensor)
        return output_nn_data

    def _copy_sync_data(
        self, batched_nn_data: dai.NNData, output_nn_data: dai.NNData
    ) -> None:
