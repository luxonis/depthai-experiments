...
class LandmarksProcessing(dai.node.HostNode):
    def __init__(self) -> None:
        super().__init__()
        self.output_gaze = dai.Node.Output(
            self,
            possibleDatatypes=[
    ...
    def process(
        ...
        if not landmark_nn.getAllLayerNames():
            self._send_outputs(
                gaze_data, left_bbox_buffer, right_bbox_buffer, nose_buffer, pose_buffer
            )
            return
        ...
        for batch_index in range(batch_size):
            ...
            left_bbox = self._padded_point(
                land_data[:2], padding=30, frame_shape=frame.shape
            )
            ...
            right_bbox = self._padded_point(
                land_data[2:4], padding=30, frame_shape=frame.shape
            )
            ...
            right_img = frame[
                right_bbox[1] : right_bbox[3], right_bbox[0] : right_bbox[2]
            ]
    ...
    def _send_outputs(
        self, gaze_data, left_bbox_buffer, right_bbox_buffer, nose_buffer, pose_buffer
    ):
