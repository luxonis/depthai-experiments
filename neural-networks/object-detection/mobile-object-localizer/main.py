...
parser.add_argument(
    "-t", "--threshold", type=float, help="Confidence threshold", default=0.2
)
...
with dai.Pipeline(device) as pipeline:
    ...
    preview = cam.requestOutput(
        (PREVIEW_WIDTH, PREVIEW_HEIGHT), dai.ImgFrame.Type.RGB888i
    )
    ...
    object_localizer = pipeline.create(ObjectLocalizer).build(
        cam=preview, nn=detection_nn.out, manip=nn_input
    )
