...
class ObjectLocalizer(dai.node.HostNode):
    ...
    def build(
        self, cam: dai.Node.Output, nn: dai.Node.Output, manip: dai.Node.Output
    ) -> "ObjectLocalizer":
    ...
    def process(
        self, cam: dai.ImgFrame, nn: dai.ImgDetections, manip: dai.ImgFrame
    ) -> None:
        ...
        for i, detection in enumerate(detections):
            ...
            color = (
                int(self._colors_full[i, 0]),
                int(self._colors_full[i, 1]),
                int(self._colors_full[i, 2]),
            )
    ...
    def _draw_boxes(
        self, frame: np.ndarray, detection: np.ndarray, color, scores
    ) -> None:
        color_black = (0, 0, 0)
