...
class DepthMerger(dai.node.HostNode):
    ...
    def _detection_to_spatial(
        ...
        # shrink the roi to get only body of the human
        shrinking_factor = 0.1
        xmin += (xmax - xmin) * shrinking_factor
        ymin += (ymax - ymin) * shrinking_factor
        xmax -= (xmax - xmin) * shrinking_factor
        ymax -= (ymax - ymin) * shrinking_factor
        roi = [
