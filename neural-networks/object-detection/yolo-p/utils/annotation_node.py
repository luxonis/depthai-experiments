...
class AnnotationNode(dai.node.ThreadedHostNode):
    def __init__(
        ...
        self.input_road_segmentations = self.createInput()
        self.input_lane_segmentations = self.createInput()
    ...
    def run(self):
        while self.isRunning():
            detections_message: ImgDetectionsExtended = self.input_detections.get()
            road_segmentations_message: SegmentationMask = (
                self.input_road_segmentations.get()
            )
            lane_segmentations_message: SegmentationMask = (
                self.input_lane_segmentations.get()
            )
