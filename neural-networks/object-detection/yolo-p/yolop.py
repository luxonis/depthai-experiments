...
class YoloP(dai.node.HostNode):
    ...
    def process(self, img_frame: dai.Buffer, nn_data: dai.NNData) -> None:
        ...
        area = (
            nn_data.getTensor("drive_area_seg")
            .astype(np.float16)
            .reshape((1, 2, self._nn_height, self._nn_width))
        )
        lines = (
            nn_data.getTensor("lane_line_seg")
            .astype(np.float16)
            .reshape((1, 2, self._nn_height, self._nn_width))
        )
        ...
        boxes = np.array(
            non_max_suppression(dets, self._confidence_threshold, self._iou_threshold)[
                0
            ]
        )
        ...
        cv2.rectangle(
            manip_frame,
            (0, manip_frame.shape[0] - h1 - 6),
            (w1 + 2, manip_frame.shape[0]),
            color_white,
            -1,
        )
        cv2.putText(
