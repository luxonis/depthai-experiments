...
class CollisionAvoidance:
    def __init__(
        ...
        collision_trajectory_threshold=0.05,
        collision_time_to_impact=4,
    ):
    ...
    def parse(self, tracker_objects):
        ...
        for key in tracker_objects.keys():
            item = {"timestamp": time.time(), "value": tracker_objects[key]}
            if key not in self.entries:
                self.entries[key] = [item]
            else:
                self.entries[key] = (self.entries[key] + [item])[
                    -self.calculated_entries :
                ]

            if (
                len(self.entries[key]) > 2
                and self.is_dangerous_trajectory(key)
                and self.is_impact_close(key)
            ):
                return True
            else:
                return False
