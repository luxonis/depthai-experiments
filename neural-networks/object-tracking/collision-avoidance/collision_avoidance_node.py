...
class CollisionAvoidanceNode(dai.node.HostNode):
    ...
    def build(
        self, img_frames: dai.Node.Output, nn: dai.Node.Output
    ...
    def process(
        self, img_frame: dai.ImgFrame, img_detections: dai.SpatialImgDetections
    ) -> None:
        frame: np.ndarray = img_frame.getCvFrame()
        detections = img_detections.detections
        pts = [
            (item.spatialCoordinates.x, item.spatialCoordinates.z)
        ...
        if crash_alert:
            cv2.putText(
                frame,
                "DANGER!",
                (frame.shape[0] // 2, frame.shape[1] // 2 - 20),
        ...
        for detection in detections:
            ...
            cv2.putText(
                frame,
                "x: {}".format(round(detection.spatialCoordinates.x, 1)),
                (left, top + 30),
                cv2.FONT_HERSHEY_TRIPLEX,
                0.5,
                255,
            ...
            cv2.putText(
                frame,
                "y: {}".format(round(detection.spatialCoordinates.y, 1)),
                (left, top + 50),
                cv2.FONT_HERSHEY_TRIPLEX,
                0.5,
                255,
            ...
            cv2.putText(
                frame,
                "z: {}".format(round(detection.spatialCoordinates.z, 1)),
                (left, top + 70),
                cv2.FONT_HERSHEY_TRIPLEX,
                0.5,
                255,
        ...
        for key in list(self._tracker.history.keys()):
            ...
            draw_pts = np.array(
                [(self._calc_x(pt[0]), self._calc_z(pt[1])) for pt in points]
            )
    ...
    def _calc_x(self, val: float, padding: int = 0) -> tuple[int, int] | int:
        ...
        center = (
            ...
            * self._distance_bird_frame.shape[1]
        )
        if padding:
    ...
    def _calc_z(self, val: float, padding: int = 0) -> tuple[int, int] | int:
        ...
        ) * self._distance_bird_frame.shape[0]
        if padding:
