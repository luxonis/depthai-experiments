...
class DetectionsRecognitionsSync(dai.node.ThreadedHostNode):
    ...
    def run(self) -> None:
        while self.isRunning():
            # print("[DetRectSync] get host_sync/passthrough_input")

            passthrough = self.passthrough_input.get()
            # print(f"[DetRectSync {passthrough.getSequenceNum()}] got host_sync/passthrough_input with ts {passthrough.getTimestamp()}")

            # print("[DetRectSync] get host_sync/detections_input")
            detections_message = self.detections_input.get()
            # print(f"[DetRectSync {detections_message.getSequenceNum()}] got host_sync/detections_input with ts {detections_message.getTimestamp()}")

            message_group = dai.MessageGroup()
            det_ts = detections_message.getTimestamp()

            recognitions = []
            # print(f"[DetRectSync {detections_message.getSequenceNum()}] Got {len(detections_message.detections)} detections")
            for i, detection in enumerate(detections_message.detections):
                # print(f"[DetRectSync {detections_message.getSequenceNum()}] get host_sync/recognition_input for detection {i}")
                recognition_output = self.recognitions_input.get()
                # print(f"[DetRectSync {recognition_output.getSequenceNum()}] got host_sync/recognition_input with ts {recognition_output.getTimestamp()}")
                ...
                if rec_ts == det_ts:
                    ...
                    # print(f"[DetRectSync {recognition_output.getSequenceNum()}] Recognition message is not in sync with detection message. Rec ts: {rec_ts}, Det ts: {det_ts}")
                    continue
            ...
            # print(f"[DetRectSync] sent detections with ts: {det_ts}")
