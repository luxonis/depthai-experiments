...
classes = [ 'background', # not coco, but required for segmentation
    'person','bicycle','car','motorcycle','airplane','bus','train','truck','boat','traffic light','fire hydrant','stop sign','parking meter','bench','bird','cat','dog','horse','sheep','cow','elephant','bear','zebra','giraffe','backpack',
    'umbrella','handbag','tie','suitcase','frisbee','skis','snowboard','sports ball','kite','baseball bat','baseball glove','skateboard','surfboard','tennis racket','bottle','wine glass','cup','fork','knife','spoon','bowl','banana','apple','sandwich','orange','broccoli',
    'carrot','hot dog','pizza','donut','cake','chair','couch','potted plant','bed','dining table','toilet','tv','laptop','mouse','remote','keyboard','cell phone','microwave','oven','toaster','sink',
    'refrigerator','book','clock','vase','scissors','teddy bear','hair drier','toothbrush']
colors = get_colors(len(classes) - 1, 0.3)
colors.insert(0, (0, 0, 0))  # Background color
arr = [viewer.AnnotationInfo(id=i, label=classes[i], color=c) for i, c in enumerate(colors)]
...
with dai.Pipeline() as pipeline:
    ...
    nn_archive = dai.NNArchive(dai.getModelFromZoo(dai.NNModelDescription(
    ...
    detection_nn = pipeline.create(dai.node.NeuralNetwork).build(
        cam.requestOutput((640,480), type=dai.ImgFrame.Type.BGR888i, fps=FPS, resizeMode=dai.ImgResizeMode.STRETCH),
        nn_archive
    )
    # Set up the parser
    ...
    stereo.setExtendedDisparity(True) # Extended disparity consumes a lot of resources (bottlenecks to 13FPS)
    stereo.setDepthAlign(dai.CameraBoardSocket.CAM_B)

    monoLeft.requestOutput((1280,800), type=dai.ImgFrame.Type.NV12, fps=FPS).link(stereo.left)
    monoRight.requestOutput((1280,800),type=dai.ImgFrame.Type.NV12, fps=FPS).link(stereo.right)

    sync = pipeline.create(dai.node.Sync)
    sync.setRunOnHost(True)
    sync.setSyncThreshold(timedelta(seconds=1 / FPS))

    rgb_800 = cam.requestOutput((1280, 960), fps=FPS, type=dai.ImgFrame.Type.NV12, resizeMode=dai.ImgResizeMode.STRETCH)
    ...
    monoLeft.requestOutput((640,400), type=dai.ImgFrame.Type.NV12, fps=FPS).link(sync.inputs['left'])
    ...
    intrinsics = np.array(calibData.getCameraIntrinsics(dai.CameraBoardSocket.CAM_A, dai.Size2f(1280, 960)))
    rgb_distortion = np.array(calibData.getDistortionCoefficients(dai.CameraBoardSocket.CAM_A))
    mapX, mapY = cv2.initUndistortRectifyMap(
        intrinsics, rgb_distortion, None, intrinsics, (1280,960), cv2.CV_32FC1
    )

    viewer.log_rigid3("CAM_A", child_from_parent=([0, 0, 0], [1, 0, 0, 0]), xyz="RDF", timeless=True)
    viewer.log_pinhole("CAM_A/Image", child_from_parent=intrinsics, width=1280, height=960, timeless=True)
    ...
    while pipeline.isRunning():
        ...
        viewer.log_rects("NN/Image/Color/Detections",
        ...
        if 0 < len(nn.masks):
            ...
            for i, det in enumerate(nn.detections):
                mask[nn.masks == i] = det.label + 1  # Use `det.id` to identify regions and map them to `det.label`
