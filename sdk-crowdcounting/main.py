...
def callback(packet: DetectionPacket):
    ...
    cv2.putText(packet.frame, f'Predicted count: {count:.2f}', (2, 12), cv2.FONT_HERSHEY_TRIPLEX, 0.4, (0, 0, 0))

    output = np.array(lay1) * 255
    output = output.astype(np.uint8)
    output_colors = cv2.applyColorMap(output, cv2.COLORMAP_VIRIDIS)
    output_colors = cv2.resize(output_colors, (packet.frame.shape[1], packet.frame.shape[0]), interpolation=cv2.INTER_LINEAR)
