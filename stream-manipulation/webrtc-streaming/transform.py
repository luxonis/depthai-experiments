...
class VideoTransform(VideoStreamTrack):
    def __init__(self, pipeline, application, pc_id, options):
        ...
        self.preview, self.nn, self.max_disparity = start_pipeline(
...
def start_pipeline(pipeline: dai.Pipeline, options):
    ...
    preview_q = (stereo.disparity if depth_flag else cam.preview).createOutputQueue(
        blocking=False, maxSize=4
    )
