...
with dai.Pipeline(device) as pipeline:
    ...
    rgb_dets = pipeline.create(DrawDetections).build(frame=preview, nn=normalized_rgb_dets.output, label_map=label_map)
    rgb = pipeline.create(Display).build(frames=rgb_dets.output)
    rgb.setName("RGB")
    
    normalized_crop_dets = pipeline.create(NormalizeBbox).build(frame=crop.out, nn=nn.out)
    crop_dets = pipeline.create(DrawDetections).build(frame=crop.out, nn=normalized_crop_dets.output, label_map=label_map)
